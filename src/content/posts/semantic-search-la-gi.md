...
> **Series "Gỡ rối chữ semantic"** — [xem mục lục đầy đủ](/memo/posts/chu-semantic-mot-chu-nhieu-nghia/)
...
Hồi tôi mới bắt đầu xây tính năng tìm kiếm cho một sản phẩm nội bộ, có một cậu junior chạy vào hỏi: _"anh ơi tool search của mình dùng cái gì, em thấy nó không có semantic search?"_ Tôi hỏi lại: _"semantic search là sao?"_ Cậu ta trả lời: _"thì... như AI ấy, thông minh hơn grep."_
...
Tôi viết một chương trình Go nhỏ minh hoạ đúng cơ chế này. Trước khi chạy, có vài điều cần nói thẳng:
...
Output thật (verbatim từ máy tôi):
...
Trong demo tôi đặt tay vector cho đẹp. Ngoài đời, không ai làm vậy — không ai ngồi nghĩ "cái câu này thì chiều 0 = 0.85, chiều 1 = 0.75...". Thực tế, không gian có nhiều chiều hơn nhiều, và **mỗi chiều không có nghĩa cụ thể** như tôi đặt trong demo.

Vector đến từ **mô hình embedding** — một mạng nơ-ron được huấn luyện để map câu/đoạn văn vào không gian vector sao cho câu cùng nghĩa thì gần nhau. Ví dụ được nghiên cứu kỹ là [Sentence-BERT (SBERT)](https://arxiv.org/pdf/1908.10084) — một mô hình được huấn luyện để tạo ra vector câu (sentence embedding) sao cho câu cùng nghĩa thì cosine similarity cao.

> [!NOTE]
> Tôi sẽ không kể số benchmark ở đây — các con số thay đổi theo dataset, task, và phiên bản mô hình. Điều tôi muốn bạn nhớ là **cơ chế**: embedding + cosine. Còn chọn mô hình nào thì phụ thuộc bài toán cụ thể của bạn.

Trong hệ thống thực, luồng thường như sau:

1. Lúc đánh index: chạy từng tài liệu qua mô hình embedding → lưu vector vào **vector database (cơ sở dữ liệu vector)** hoặc một cấu trúc dữ liệu hỗ trợ tìm kiếm láng giềng gần (approximate nearest neighbor — ANN).
...
Như tôi nói ở [mục lục series](/memo/posts/chu-semantic-mot-chu-nhieu-nghia/): hai cái này như "con chuột" trên bàn và "con chuột" trong hang — cùng nhãn chữ, khác loài hoàn toàn.
