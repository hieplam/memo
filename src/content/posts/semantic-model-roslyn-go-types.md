...
> **Series "Gỡ rối chữ semantic"** — bạn đang đọc Phần 3.
...
- **Go demo chạy thật:** cùng chữ `x` viết 4 lần (2 khai báo + 2 chỗ dùng), nhưng shadowing tạo 2 biến khác loại (`int` / `string`). `go/types` phân giải từng chỗ dùng vào đúng biến, lưu kết quả trong **`types.Info`** — một side-map tách hẳn cây AST.
...
Hồi tôi mới tiếp cận Roslyn, tôi đọc thấy câu "Roslyn có semantic model" rồi tự hỏi: _model đó nằm ở đâu, trong cái cây AST ấy à?_ Tra mấy bài blog thì thấy người ta hay nói kiểu "semantic analysis **tô màu** lên AST" — annotated AST, decorated AST. Nghe xuôi. Nhưng sai.
...
AST (abstract syntax tree — cây cú pháp trừu tượng) nhìn vào đây thấy gì? Nó thấy: có hai phép khai báo (`:=`), hai lời gọi `fmt.Println`, và bốn node `Ident` cùng tên `"x"` (2 nút khai báo + 2 nút tham chiếu). Đó là tất cả. Cấu trúc ngữ pháp hoàn toàn ổn.
...
Đây là điểm hay bị blog nói sai nhất, nên tôi sẽ nói thẳng.

`go/types` **không chỉnh sửa AST**. Nó để kết quả vào `types.Info` — một cấu trúc dữ liệu **riêng**, tách hẳn cây. [(pkg.go.dev/go/types)](https://pkg.go.dev/go/types)
...
> [!WARNING]
> **`types.Info.Types` KHÔNG chứa kiểu của mọi identifier.** Selector `x.f` đi vào `Selections`. Khai báo `var z int` đi vào `Defs`. Package `p` trong `p.X` đi vào `Uses`. Nhiều bài blog nói "bạn tra `Types[ident]` là ra kiểu" — không đúng với mọi trường hợp. [(Alan Donovan, go-types guide)](https://github.com/golang/example/blob/master/gotypes/go-types.md)
...
Roslyn (C#) đặt tên rõ hơn cho ba tầng này. Tôi sẽ dùng đây là minh hoạ đối chiếu — máy tôi không có .NET nên mấy đoạn C# bên dưới là **minh hoạ, đối chiếu Microsoft Learn**, không phải output chạy thật.
...
Điểm chung quan trọng nhất: **cả hai đều giữ cây cú pháp nguyên vẹn**, không gắn thêm metadata vào nó. Kết quả semantic sống trong cấu trúc riêng.

## 6. Tại sao điều này quan trọng với bài tiếp theo

Bạn sẽ hỏi: "Ok, biết `go/types` lưu kết quả vào side-map, thì sao?"
